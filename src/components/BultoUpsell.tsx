...
import { PESO_MAX_BULTO_KG } from '@/lib/shipping-zones';

interface BultoUpsellProps {
    onAddToCart?: (productId: string, size: ProductSize, price: number) => void;
}

/**
 * BultoUpsell — Smart Upselling por espacio disponible en bulto
 * PRD 2026: "te quedan Xkg en tu paquete. Agrega [producto] y el envío te sigue costando igual."
 */
export default function BultoUpsell({ onAddToCart }: BultoUpsellProps) {
    const { cart, getTotalWeightKg, addToCart, getTotalPrice } = useCart();

    const totalKg = getTotalWeightKg();
    const kgRestantes = PESO_MAX_BULTO_KG - (totalKg % PESO_MAX_BULTO_KG || PESO_MAX_BULTO_KG);
    const bultos = Math.ceil(totalKg / PESO_MAX_BULTO_KG);

    // Mostrar sugerencia solo si quedan ≥3kg en el bulto actual y el carrito no está vacío
    if (cart.length === 0 || kgRestantes < 3 || kgRestantes >= PESO_MAX_BULTO_KG) return null;

    // Encontrar el producto + talla más grande que quepa en el espacio disponible
    ...
    for (const producto of PRODUCTOS) {
        // No sugerir un producto que ya está en el carrito en esa misma talla
    ...
    // Elegir la sugerencia que llene mejor el espacio (mayor peso que quepa)
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-black text-amber-800 mb-0.5 uppercase tracking-wide">
                            🚚 Aprovecha el espacio en tu bulto
                        </p>
                        <p className="text-xs text-amber-700 leading-snug mb-3">
                            Te quedan <span className="font-black">{kgRestantes.toFixed(0)}kg</span> libres
